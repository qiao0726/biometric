...
class LSTM(nn.Module):
    def __init__(self, in_feat_dim=1, hidden_state_dim=32, num_layers=2, use_all_outputs=False,
                 ...
                 data_type='sensor'):
        """ LSTM model for sensor and touch screen data.
        ...
        Args:
            ...
            bidirectional (bool, optional): if use bi-LSTM. Defaults to False.
        ...
        if data_type == 'touchscreen':
            self.lstm = nn.LSTM(input_size=len(self.apply_data_list)*in_feat_dim, hidden_size=hidden_state_dim,
                                   num_layers=num_layers, bidirectional=bidirectional, batch_first=True)
        elif data_type == 'sensor':
            self.lstm = nn.LSTM(input_size=in_feat_dim, hidden_size=hidden_state_dim,
                                   num_layers=num_layers, bidirectional=bidirectional, batch_first=True)
        else:
    ...
    def forward_ts(self, ts_data):
        ...
        output = self.lstm(concat_tensor)
        output = output[0]
